...
$id = $_GET['id'];
$sql = "SELECT * from request_list WHERE rid = '$id'";
		$result = $Con->execute ( $sql );
		if (! $result->EOF){
		$priv_id = $result->Fields('rid')->Value;
		$pri_name = $result->Fields('rtitle')->Value;
		}
?>
<div class="row">
	<div class="col-lg-12">
		...
		<div class="page-header title">
			<!-- PAGE TITLE ROW -->
			<h1>Settings<span class="sub-title">Request</span></h1>
		</div>
			...
			<div id="f-6" class="panel-collapse collapse in">
				<div class="portlet-body">
						...
						<div class="form-group">
							<label>Request</label>
							<input type="text" class="form-control" name="request" value="<?php echo $pri_name ; ?>">
						</div>

							<input type="hidden" class="form-control" name="priv_id" value="<?php echo $priv_id ; ?>">
						<div id="processDiv"></div>
						<div class="form-actions">
							<button type="button" class="btn btn-primary"
							onclick="showAjax('Page_Form','../includes/ajaxaction1.php?Action=updateReq','processDiv')"
							>Update</button>
							<button type="submit" class="btn"
							onclick="showAjax('Page_Form','../includes/ajaxaction1.php?Action=cancelReq','processDiv')"
							>Cancel</button>
						</div>
