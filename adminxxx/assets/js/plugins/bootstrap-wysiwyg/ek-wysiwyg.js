/**
 A wrapper for Bootstrap wyswiwyg plugin.
 It's just a wrapper so you still need to include Bootstrap wysiwyg script first.
*/
(function($ , undefined) {
	$.fn.ek_wysiwyg = function($options , undefined) {
		var options = $.extend( {
			speech_button:true,
			wysiwyg:{}
        }, $options);
		...
		var button_defaults =
		{
			...
			'insertImage' : {
				...
				button_text : '<i class="fa fa-file"></i>Choose &hellip;',
				placeholder : 'Image URL',
				button_insert : 'Insert',
				button_class : 'btn-default',
				button_insert_class : 'btn-primary',
				choose_file: true //show the choose file button "false" for disable
			},
		...
		this.each(function() {
			var toolbar = ' <div class="wysiwyg-toolbar btn-toolbar"> <div class="btn-group"> ';
			...
			for(var tb in toolbar_buttons) if(toolbar_buttons.hasOwnProperty(tb)) {
				...
				switch(button.name) {
					...
					case 'createLink':
						toolbar += ' <div class="btn-group"> <a class="btn btn-sm '+className+' dropdown-toggle" data-toggle="dropdown" title="'+button.title+'"><i class="'+ button.icon+'"></i></a> ';
						toolbar += ' <div class="dropdown-menu dropdown-caret dropdown-menu-right">\
							 <div class="input-group">\
								...
								<span class="input-group-btn">\
									<button class="btn '+button.button_class+'" type="button">'+button.button_text+'</button>\
								</span>\
							 </div>\
						</div> </div>';
					break;
					...
					case 'insertImage':
						...
						toolbar += ' <div class="dropdown-menu dropdown-caret dropdown-menu-right">\
							 <div class="input-group">\
								<input class="form-control" placeholder="'+button.placeholder+'" type="text" data-edit="'+button.name+'" />\
								<span class="input-group-btn">\
									<button class="btn '+button.button_insert_class+'" type="button">'+button.button_insert+'</button>\
								</span>\
							 </div>';
							if( button.choose_file && 'FileReader' in window ) toolbar +='\
							 <label class="center block no-margin-bottom">\
								<button class="btn btn-xs '+button.button_class+' wysiwyg-choose-file" type="button">'+button.button_text+'</button>\
